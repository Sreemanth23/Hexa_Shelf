...
export class TileCreation extends Component {
  ...
  placingShelf() {
    for (let i = 0; i < this.numberOfSets; i++) {
      ...
      shelfContainer.setPosition(0, -0.47 + i * 0.3, 0);
    }
    this.tileCreation();
  }
      ...
      for (let j = 0; j < 18; j++) {
        ...
        for (let i = 0; i < 5; i++) {
          if (!(k >= 1 && j == 0)) {
            ...
            tile.getComponent(MeshRenderer).setMaterial(this.colorMaterial[selectColor],0);
            //.materials[0] = this.colorMaterial[selectColor];
